...
class EagleManager:
    ...
    DLL_MAPPINGS = {
        ...
        "d3d10.dll": ("DirectX 10", "Graphics"),
        "d3d11.dll": ("DirectX 11", "Graphics"),
        "d3d12.dll": ("DirectX 12", "Graphics"),
        "dxgi.dll": ("DXGI", "Graphics"),
        "d3dcompiler_47.dll": ("D3D Compiler", "Graphics"),
        "opengl32.dll": ("OpenGL", "Graphics"),
        "vulkan-1.dll": ("Vulkan", "Graphics"),
        "nvapi.dll": ("NVAPI", "Graphics"),
        "nvapi64.dll": ("NVAPI", "Graphics"),
        "amd_ags_x64.dll": ("AMD AGS", "Graphics"),
        "physx_loader.dll": ("PhysX", "Graphics"),
        "physx3_x64.dll": ("PhysX", "Graphics"),
        "d3dx9_43.dll": ("D3DX9", "Graphics"),
        "d3dx11_43.dll": ("D3DX11", "Graphics"),
        "nvngx_dlss.dll": ("DLSS", "Upscaling"),
        "nvngx_dlssg.dll": ("DLSS 3 Frame Gen", "Upscaling"),
        "libxess.dll": ("Intel XeSS", "Upscaling"),
        "ffx_fsr2_api_x64.dll": ("AMD FSR 2", "Upscaling"),
        "amd_fidelityfx_dx12.dll": ("AMD FSR", "Upscaling"),
        "amd_fidelityfx_vk.dll": ("AMD FSR", "Upscaling"),
        "xaudio2_7.dll": ("XAudio 2.7", "Audio"),
        "xaudio2_9.dll": ("XAudio 2.9", "Audio"),
        "x3daudio1_7.dll": ("X3DAudio", "Audio"),
        "fmod.dll": ("FMOD", "Audio"),
        "fmod64.dll": ("FMOD", "Audio"),
        "fmodstudio.dll": ("FMOD Studio", "Audio"),
        "wwise.dll": ("Wwise", "Audio"),
        "binkw32.dll": ("Bink", "Audio"),
        "binkw64.dll": ("Bink", "Audio"),
        "openal32.dll": ("OpenAL", "Audio"),
        "dsound.dll": ("DirectSound", "Audio"),
        "miles32.dll": ("Miles", "Audio"),
        ...
        "msvcp100.dll": ("VC++ 2010", "Runtimes"),
        "msvcr100.dll": ("VC++ 2010", "Runtimes"),
        "msvcp110.dll": ("VC++ 2012", "Runtimes"),
        "msvcr110.dll": ("VC++ 2012", "Runtimes"),
        "msvcp120.dll": ("VC++ 2013", "Runtimes"),
        "msvcr120.dll": ("VC++ 2013", "Runtimes"),
        "msvcp140.dll": ("VC++ 2015-22", "Runtimes"),
        "vcruntime140.dll": ("VC++ 2015-22", "Runtimes"),
        ...
        "python311.dll": ("Python 3.11", "Runtimes"),
        "lua54.dll": ("Lua 5.4", "Runtimes"),
        "xinput1_3.dll": ("XInput 1.3", "Input"),
        "xinput1_4.dll": ("XInput 1.4", "Input"),
        "dinput8.dll": ("DirectInput", "Input"),
        "sdl2.dll": ("SDL2", "Input"),
        "steam_api.dll": ("Steamworks", "Social/DRM"),
        "steam_api64.dll": ("Steamworks", "Social/DRM"),
        "galaxy.dll": ("GOG Galaxy", "Social/DRM"),
        ...
        "easyanticheat.dll": ("EAC", "Protection"),
        "easyanticheat_x64.dll": ("EAC", "Protection"),
        "battleye.dll": ("BattlEye", "Protection"),
    ...
    def _scan_yara(
        self, file_path: str, insights: dict, source: str = "Main Executable"
    ) -> list:
        ...
        try:
            ...
            for match in results:
                ...
                existing_names = (
                    [i["name"] for i in insights[category]]
                    if isinstance(insights[category], list)
                    else []
                )
                context = []
                for valid_match in match.strings:
                    ...
                    if isinstance(valid_match, tuple):
                        ...
                        try:
                            ...
                            if (
                                hasattr(valid_match, "instances")
                                and valid_match.instances
                            ):
                    ...
                    if should_capture:
                        try:
                            ...
                            if (
                                clean_upper not in roots
                                and len(clean) > 2
                                and clean not in context
                            ):
                ...
                if name not in existing_names:
                    if category in ("Warning", "Security"):
                        ...
                        insights[category].append(
                            {"name": name, "source": source, "context": context_list}
                        )
    ...
    def _extract_asar(self, asar_path: str) -> tuple:
        ...
        try:
            self._send_step(
                f"Extracting ASAR archive: {os.path.basename(asar_path)}..."
            )
                    ...
                    if "files" in dir_structure:
                        for name, info in dir_structure["files"].items():
                            ...
                            if "files" in info:
                                # Directory
                                os.makedirs(
                                    os.path.join(extract_dir, new_path), exist_ok=True
                                )
                                ...
                                try:
                                    ...
                                    logging.warning(
                                        f"[Eagle] Failed to extract {new_path}: {e}"
                                    )
        ...
        try:
                ...
                if result.returncode != 0:
                    try:
                        patoolib.extract_archive(
                            installer_path, outdir=extract_dir, verbosity=-1
                        )
                    except Exception:
            ...
            for pattern in ["**/*.exe", "**/*.dll"]:
                extracted_files.extend(
                    glob(os.path.join(extract_dir, pattern), recursive=True)
                )
        ...
        try:
            if is_msi:
                insights = {
                    ...
                    "Crypto": [],
                    "Upscaling": [],
                    "Installer": [{"name": "MSI", "source": "File Extension"}],
                    "Warning": [],
                    "Registry": [],
                ...
                extracted_files, extract_dir = self._extract_installer(
                    executable_path, "MSI"
                )
                if extracted_files:
                    ...
                    for i, ef in enumerate(files_to_scan):
                        ...
                        self._send_step(
                            f"[{i + 1}/{len(files_to_scan)}] Scanning: {fname}",
                            delay=False,
                        )
                ...
                SignalManager.send(
                    Signals.EagleFinished, Result(status=True, data=results)
                )
                return
            ...
            try:
                if hasattr(pe, "DIRECTORY_ENTRY_RESOURCE"):
                    for entry in pe.DIRECTORY_ENTRY_RESOURCE.entries:
                        if hasattr(
                            entry, "name"
                        ) and entry.name == pefile.RESOURCE_TYPE.get("RT_MANIFEST"):
            ...
            is_net = False
            try:
                clr = pe.OPTIONAL_HEADER.DATA_DIRECTORY[
            ...
            for dll in imports:
                if dll in self.DLL_MAPPINGS:
                    ...
                    if name not in existing_names:
                        insights[cat].append({"name": name, "source": "Imports"})
            ...
            if is_net:
                ...
                if "hostfxr.dll" in imports or "coreclr.dll" in imports:
                    if ".NET Core/5+" not in existing_runtimes:
                        insights["Runtimes"].append(
                            {"name": ".NET Core/5+", "source": "CLR Header"}
                        )
                elif ".NET Framework" not in existing_runtimes:
                    insights["Runtimes"].append(
                        {"name": ".NET Framework", "source": "CLR Header"}
                    )
            ...
            if os.path.exists(os.path.join(exe_dir, "Engine")) or os.path.exists(
                os.path.join(exe_dir, "../Engine/Binaries")
            ):
                ...
                if (
                    ...
                    self._send_step(
                        "[!] Unreal Engine bootstrapper detected – searching for shipping binary..."
                    )
                    try:
                            ...
                            for entry in it:
                                ...
                                for arch in ("Win64", "Win32"):
                                    binaries_path = os.path.join(
                                        entry.path, "Binaries", arch
                                    )
                                    if not os.path.isdir(binaries_path):
                                        continue
                                    for f in os.listdir(binaries_path):
                                        if f.lower().endswith(
                                            f"-{arch.lower()}-shipping.exe"
                                        ):
                                            ue_shipping_exe = os.path.join(
                                                binaries_path, f
                                            )
                                            self._send_step(
                                                f"[Engines] Shipping binary found: {f}"
                                            )
                                            break
                                    if ue_shipping_exe:
                                        break
            ...
            if os.path.exists(os.path.join(exe_dir, "www")) or os.path.exists(
                os.path.join(exe_dir, "Game.rgss3a")
            ):
            ...
            if os.path.exists(os.path.join(exe_dir, "resources/app.asar")):
                ...
                if extracted_asar_files:
                    ...
                    for i, ef in enumerate(extracted_asar_files[:scan_limit]):
                        ...
                        self._scan_yara(
                            ef, insights, source=f"Electron Source: {fname}"
                        )
            ...
            if os.path.exists(unity_data):
                ...
                if os.path.exists(plugins_dir):
                    neighbor_files.extend(
                        glob(os.path.join(plugins_dir, "**/*.dll"), recursive=True)
                    )

            scan_limit = 20
            if neighbor_files:
                self._send_step(
                    f"Found {len(neighbor_files)} neighbor files. Ensure directory is clean to avoid false positives."
                )
                insights["Warning"].append(
                    {
                ...
                if fname not in insights["Analysed Files"] and nf != executable_path:
                    ...
                    try:
                        ...
                        if f_lower in self.DLL_MAPPINGS:
                            ...
                            if name not in existing_names:
                                insights[cat].append(
                                    {"name": name, "source": f"Neighbor: {fname}"}
                                )
                    except:
                        pass
            ...
            if insights["Installer"]:
                ...
                installer_name = (
                    installer_item["name"]
                    if isinstance(installer_item, dict)
                    else str(installer_item)
                )
                self._send_step(
                    f"Installer detected ({installer_name}), performing deep scan..."
                )
                try:
                    extracted_files, extract_dir = self._extract_installer(
                        executable_path, installer_name
                    )
                    if extracted_files:
                        exe_files = [
                            f for f in extracted_files if f.lower().endswith(".exe")
                        ]
                        if exe_files:
                            main_exe = max(exe_files, key=os.path.getsize)
                            self._send_step(
                                f"Scanning main binary: {os.path.basename(main_exe)}"
                            )
                    ...
                    self._send_step(
                        "Deep scan failed, continuing with surface analysis"
                    )
            ...
            dx = [
                v
                for v in flat_insights.get("Graphics", [])
                if "DirectX" in v or "D3D" in v or "DXGI" in v
            ]
            has_dx12 = any("12" in v for v in dx)
            has_dx11 = any(
                "11" in v or "9" in v or "10" in v or "DXGI" in v for v in dx
            )
            ...
            has_dx_engine = any(
                e in flat_insights.get("Engines", []) for e in dx_engines
            )

            dx12_engines = ["Unreal", "REDengine"]
            has_dx12_engine = any(
                e in flat_insights.get("Engines", []) for e in dx12_engines
            )

            if has_dx12 or has_dx12_engine:
                suggestions.append(
                    {"key": "vkd3d", "value": True, "label": "Enable VKD3D (DX12)"}
                )
            if has_dx11 or (has_dx_engine and not has_vulkan):
                suggestions.append(
                    {"key": "dxvk", "value": True, "label": "Enable DXVK (DX9-11)"}
                )
                ...
                suggestions.append(
                    {
                        "key": "dxvk",
                        "value": False,
                        "label": "DXVK not needed (native Vulkan)",
                    }
                )

            if "NVAPI" in flat_insights.get("Graphics", []):
                suggestions.append(
                    {"key": "dxvk_nvapi", "value": True, "label": "Enable DXVK-NVAPI"}
                )
                ...
                suggestions.append(
                    {
                        "key": "dxvk_nvapi",
                        "value": True,
                        "label": "Enable DXVK-NVAPI (for DLSS)",
                    }
                )
                ...
                suggestions.append(
                    {
                        "key": "fsr",
                        "value": True,
                        "label": "Enable FSR (native support detected)",
                    }
                )

            if flat_insights.get("Engines", []) or flat_insights.get("Graphics", []):
                suggestions.append(
                    {"key": "gamemode", "value": True, "label": "Enable GameMode"}
                )
                suggestions.append(
                    {"key": "discrete_gpu", "value": True, "label": "Use discrete GPU"}
                )
                if ntsync_available:
                    suggestions.append(
                        {
                            "key": "sync",
                            "value": "ntsync",
                            "label": "NTsync (threading)",
                        }
                    )
                else:
                    suggestions.append(
                        {"key": "sync", "value": "esync", "label": "Esync (threading)"}
                    )
            ...
            if has_mono and net_runtimes:
                ...
                suggestions.append(
                    {
                        "key": "dep_mono",
                        "value": False,
                        "label": "Wine Mono (Recommended first)",
                    }
                )
                ...
                for rt in set(net_runtimes):
                    suggestions.append(
                        {"key": "dep_dotnet", "value": False, "label": rt}
                    )
                if has_mono and not net_runtimes:
                    suggestions.append(
                        {"key": "dep_mono", "value": False, "label": "Wine Mono"}
                    )
                ...
                suggestions.append(
                    {
                        "key": "dep_vcredist",
                        "value": False,
                        "label": f"Visual C++ ({', '.join(set(vcpp))})",
                    }
                )
            ...
            SignalManager.send(
                Signals.EagleFinished, Result(status=False, data={"error": str(e)})
            )
        finally:
            if extract_dir:
